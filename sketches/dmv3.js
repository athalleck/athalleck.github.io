// ------------------ V.3A: Slow Motion, Multi-Color Motion-Based Drawing Machine ------------------
window.sketch3 = (p) => {
  const branchingFactor = 2;
  let motionFactor = 50;  // reduced for slower movement
  let agentSpeed = 20;    // base speed
  const agentArray = [];
  let world;
  let video;
  let prevFrame;
  const motionThreshold = 50; 
  const fadeAmount = 2;  // less fade for more persistent trails
  ...
  // --- Click to reset and start with a new random palette ---
  p.mouseClicked = () => {
    currentPaletteIndex = Math.floor(p.random(colorPalettes.length));
  ...
  p.draw = () => {
    ...
    if (motionLevel > 0.01) {
      ...
      agentSpeed = p.map(motionLevel, 0.01, 1, 5, 40);  // slower max speed
    } else {
      agentSpeed = 10;
    }

    if (agentSpeed > 0) {
      for (let i = 0; i < Math.ceil(agentSpeed / 3); i++) { // smaller steps per frame
    ...
    if (motionLevel > 0.5 && agentArray.length < 20) {
      agentArray.push(agent());
    }
    ...
    while (stack.length) {
      ...
      for (let i = 0; i < num; i++, heading += p.random(-p.PI / 8, p.PI / 8)) {
        ...
        do {
          ...
          // Bounce off edges but allow more freedom
  ...
  function array2d() {
    ...
    return {
      ...
      set: (x, y) => {
        // Pick a random color for every single point
